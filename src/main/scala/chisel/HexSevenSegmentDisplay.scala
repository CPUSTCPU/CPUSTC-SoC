...
class HexSevenSegmentDisplayIO extends Bundle {
  val value: UInt = Input(UInt(32.W))
  val csn:   UInt = Output(UInt(8.W))
  val aG:    UInt = Output(UInt(7.W))
}

/** 将 32 位数据按八位十六进制形式扫描到板载数码管。
  *
  * 该模块只实现位扫描和十六进制段码，不包含显示源选择或跨时钟域处理。
  */
class HexSevenSegmentDisplay(clockHz: Int = 33_000_000, refreshHz: Int = 63) extends Module {
  require(clockHz > 0)
  require(refreshHz > 0)

  val io: HexSevenSegmentDisplayIO = IO(new HexSevenSegmentDisplayIO)
