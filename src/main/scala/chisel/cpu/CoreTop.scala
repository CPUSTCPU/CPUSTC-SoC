...
class RawCoreTop extends ExtModule {
  ...
  val retire_valid: UInt = IO(Output(UInt(3.W)))
  val retire_pc0: UInt = IO(Output(UInt(32.W)))
  val retire_pc1: UInt = IO(Output(UInt(32.W)))
  val retire_pc2: UInt = IO(Output(UInt(32.W)))
...
class CoreTopIO extends Bundle {
  ...
  val retireValid: UInt = Output(UInt(3.W))
  val retirePc: Vec[UInt] = Output(Vec(3, UInt(32.W)))
...
class CoreTop extends RawModule {
  ...
  io.retireValid := raw.retire_valid
  io.retirePc := VecInit(raw.retire_pc0, raw.retire_pc1, raw.retire_pc2)
