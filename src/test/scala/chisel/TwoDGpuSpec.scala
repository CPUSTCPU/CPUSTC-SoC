  ...
  final class Driver(dut: TwoDGpu) {
    ...
    def configureScale(
      ...
      val writes = Seq(
        ...
        TwoDGpuRegisters.srcXy -> xy(srcX, srcY),
        TwoDGpuRegisters.dstXy -> xy(dstX, dstY),
        TwoDGpuRegisters.srcSize -> xy(srcWidth, srcHeight),
        TwoDGpuRegisters.dstSize -> xy(dstWidth, dstHeight)
      )
      writes.foreach { case (address, value) =>
        assert(!write(address, value).error, f"scale configuration write 0x$address%x failed")
      }
    }
...
class TwoDGpuSpec extends AnyFreeSpec with ChiselScalatestTester {
  ...
  "YUYV_SCALE converts BT.601 pixels and reuses source rows while scaling with AXI bursts" in {
    test(new TwoDGpu).withAnnotations(annotations) { dut =>
      dut.clock.setTimeout(0)
      val driver = new Driver(dut)
      driver.reset()

      val srcBase = BigInt(0xa000)
      val dstBase = BigInt(0xb000)
      val srcStride = 4
      val dstStride = 8
      val source = Seq(
        Seq(16, 128, 235, 128),
      ...
      for (y <- source.indices; x <- source(y).indices) {
        driver.memory.setByte(srcBase + y * srcStride + x, source(y)(x))
      }
      driver.configureScale(
        srcAddress = srcBase,
        dstAddress = dstBase,
        srcStride = srcStride,
        dstStride = dstStride,
      ...
      driver.memory.clearTransactions()
      driver.start(TwoDGpuCommands.yuyvScale)
      expectSuccess(driver)

      val expected = Seq(
        Seq(0x0000, 0x0000, 0xffff, 0xffff),
        Seq(0x0000, 0x0000, 0xffff, 0xffff),
        Seq(0xf800, 0xf800, 0xf800, 0xf800),
        Seq(0xf800, 0xf800, 0xf800, 0xf800)
      )
      for (y <- expected.indices; x <- expected(y).indices) {
        assert(driver.memory.pixel(dstBase, dstStride, x, y) == expected(y)(x),
          s"YUYV_SCALE pixel mismatch at ($x,$y)")
      }
      assert(driver.memory.readBursts.map(_.address).toSeq == Seq(srcBase, srcBase + srcStride))
      assert(driver.memory.writeBursts.size == 4)
      assert(driver.memory.writeBursts.forall(_.beats == 2))
    }
  }

  "YUYV_SCALE splits source and destination bursts at 16 beats and 4 KiB" in {
    test(new TwoDGpu).withAnnotations(annotations) { dut =>
      dut.clock.setTimeout(0)
      val driver = new Driver(dut)
      driver.reset()

      val srcBase = BigInt(0x10fc0)
      val dstBase = BigInt(0x20fc0)
      val width = 34
      val stride = width * 2
      for (x <- 0 until width by 2) {
        val address = srcBase + x * 2

        driver.memory.setByte(address, 16)
        driver.memory.setByte(address + 1, 128)
        driver.memory.setByte(address + 2, 235)
        driver.memory.setByte(address + 3, 128)
      }
      driver.configureScale(
        srcAddress = srcBase,
        dstAddress = dstBase,
      ...
      for (x <- 0 until width) {
        val expected = if ((x & 1) == 0) 0x0000 else 0xffff

        assert(driver.memory.pixel(dstBase, stride, x, 0) == expected,
          s"YUYV_SCALE burst pixel mismatch at $x")
      }
      assert(driver.memory.readBursts.map(_.beats).toSeq == Seq(16, 1))
      assert(driver.memory.writeBursts.map(_.beats).toSeq == Seq(16, 1))
    }
  }
