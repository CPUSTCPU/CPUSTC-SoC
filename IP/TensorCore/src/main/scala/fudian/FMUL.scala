...
class FMUL(val expWidth: Int, val precision: Int) extends Module {
  ...
  // TensorPE's valid path assumes a two-cycle multiplier latency.  Keep the
  // arithmetic stages aligned by registering both the control path and the
  // significand product at each stage boundary.
  fmul_s2.io.in := RegNext(fmul_s1.io.out)
  fmul_s2.io.prod := RegNext(multiplier.io.result)
